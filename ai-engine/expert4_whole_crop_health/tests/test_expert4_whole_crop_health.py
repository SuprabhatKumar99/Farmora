...
class FakeWholeCropHealthModel:
    name = "fake-expert4"
    version = "test-1"
    task = "DETECTION"
    ...
    def predict(self, image, confidence_threshold):
        return [
            {
                "class_id": 1,
                "class_name": "whole_crop_health_pattern",
                "confidence": 0.90,
                "bbox": [2, 3, 20, 21],
            }
        ]
...
def test_expert4_service_with_test_model(tmp_path):
    path = tmp_path / "sample.jpg"
    create_image(path)

    service = Expert4WholeCropHealthService(
        model=FakeWholeCropHealthModel()
    )
    ...
    assert result.expert == "EXPERT_4_WHOLE_CROP_HEALTH"
